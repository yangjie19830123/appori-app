...
import { formatWageCompact, localizedCountry, textOnAccent } from "../lib/countries";
...
export default function InputCard({
  ...
  const wageDisplay = hourlyWage > 0 ? localToDisplay(hourlyWage, country.code, rates, lang) : 0;
  const accentTxt = textOnAccent(country.accentColor);
  ...
  return (
    <div className="rounded-3xl bg-white border-[3px] border-ink shadow-stamp-lg p-6 space-y-6 animate-fade-up relative overflow-hidden">
      <div
        ...
        style={{ background: country.accentColor, color: accentTxt.primary }}
      >
        {country.emoji} {lc.name}
      </div>

      {/* 时薪 */}
      <div>
        <div className="flex items-baseline justify-between mb-3">
          <label className="text-sm font-bold text-ink">{t.inputWageLabel}</label>
          <span className="text-[11px] text-ink-mute">
            {t.inputWageMin} <span className="font-mono font-bold">{formatWageCompact(country.minWage, country)}</span>
          </span>
        </div>
          ...
          {wagePresets.map((v) => (
            <button
              ...
              className={`press-stamp px-3.5 py-1.5 rounded-full text-xs font-bold border-2 border-ink transition ${
                hourlyWage === v ? "shadow-stamp-sm" : "bg-white shadow-stamp-sm hover:bg-cream-100"
              }`}
              style={hourlyWage === v ? { background: country.accentColor, color: accentTxt.primary } : undefined}
            >
      ...
      <div>
        <div className="flex items-baseline justify-between mb-3">
          <label className="text-sm font-bold text-ink">{t.inputHoursLabel}</label>
        ...
        <div className="flex justify-between mt-2 text-[10px] text-ink-mute font-mono">
          ...
          {isStudent && (
            ...
            >
              {t.inputHoursStudentLimit.replace("{n}", String(country.studentHourLimit))}
            </span>
          )}
          <span>60h</span>
        </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-2xl bg-cream-100 border-[3px] border-ink shadow-stamp-sm">
        <div className="min-w-0 mr-3">
          <div className="text-sm font-bold text-ink">{t.inputStudentTitle}</div>
          <div className="text-[11px] text-ink-mute mt-0.5 leading-snug">{lc.studentHourNote}</div>
        </div>
        <button
          type="button"
          role="switch"
          aria-checked={isStudent}
          aria-label={t.inputStudentToggleLabel}
